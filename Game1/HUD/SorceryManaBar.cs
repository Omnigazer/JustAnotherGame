...
namespace Omniplatformer.HUD
{
    ...
    {
        public SorceryManaBar(Point position, int width, int height) : base(position, width, height)
        {
        }

        public override void Draw()
        {
            if (Player.MaxMana(ManaType.Sorcery) > 0)
            {
                ...
                Point current_size = new Point((int)((Width - Thickness * 2) * (Player.CurrentMana[ManaType.Sorcery] / Player.MaxMana(ManaType.Sorcery))),
                    Height - Thickness * 2);
                ...
                spriteBatch.Draw(GameContent.Instance.testLiquid, inner_rect, source_rect, Color.Aqua);
