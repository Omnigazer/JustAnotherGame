...
namespace Omniplatformer.HUD
{
    ...
    {
        public DeathManaBar(Point position, int width, int height) : base(position, width, height)
        {
        }

        public override void Draw()
        {
            if (Player.MaxMana(ManaType.Death) > 0)
            {
                ...
                Point current_size = new Point((int)((Width - Thickness * 2) * (Player.CurrentMana[ManaType.Death] / Player.MaxMana(ManaType.Death))),
                    Height - Thickness * 2);
                ...
                spriteBatch.Draw(GameContent.Instance.testLiquid, inner_rect, source_rect, Color.DarkViolet);
