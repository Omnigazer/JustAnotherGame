...
namespace Omniplatformer.HUD
{
    ...
    {
        public LifeManaBar(Point position, int width, int height) : base(position, width, height)
        {
        }

        public override void Draw()
        {
            if (Player.MaxMana(ManaType.Life) > 0)
            {
                ...
                Point current_size = new Point((int)((Width - Thickness * 2) * (Player.CurrentMana[ManaType.Life] / Player.MaxMana(ManaType.Life))),
                    Height - Thickness * 2);
                ...
                // spriteBatch.Draw(GameContent.Instance.testLiquid, inner_rect, source_rect, Color.LightYellow);
                spriteBatch.Draw(GameContent.Instance.testLiquid, inner_rect, source_rect, Color.Azure);
